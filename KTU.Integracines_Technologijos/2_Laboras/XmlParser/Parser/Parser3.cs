    ...
    public class Parser3
    {
        public void ParseXmlUsingDataSet()
        {
                ...
                //imamos vakariniu studentu lenteles eilutes
            {
                ...
                foreach (
                    DataRow row in dataSet.Tables["matematika"].Select(string.Format("Vakarinis_Id={0}", vakarinisId)))
                    //einame per pazymiu lenteles kurios siejasi su vakarinis_id
                {
                    ...
                    vakarinisStudentas.Paz1 = (row.ItemArray[0]).ToString(); //nuskaitomas matematikos paz1
                    vakarinisStudentas.Paz2 = (row.ItemArray[1]).ToString(); //nuskaitomas matematikos paz2

                    //imamas pazymiu ir technologijos eiluciu sarysis duomenu nuskaitymui
                    vakarinisStudentas.Paz11 = (row.ItemArray[0]).ToString(); //nuskaitomas technologijos paz1
                    vakarinisStudentas.Paz22 = (row.ItemArray[1]).ToString(); //nuskaitomas technologijos paz1
                }
            ...
            foreach (DataRow dataSetRow in dataSet.Tables["Dieninis"].Rows) //imam atitinkamos lentelės eilutes
            {
                ...
                {
                    DataRow[] children = row1.GetChildRows("pazymiai_fizika");
